...
>
> **2026-08-05 重大更新（覆盖本文相关章节）**：`old-city` 与 `old-city-crossing`
> 已按所有者手绘图**合并为单一关卡**（西侧竖向马路+斑马线在关卡开头，U 形盲道
> 穿商铺街到银号门前终点接 ruins）。**扶手机制已砍掉**，过街黄点线改为斑马线瓦片
> 提亮，商铺全部瓦片体块+程序化招牌（无 AI 贴图、无真实品牌），存档升 v4。
> 本文中涉及扶手验收（§3.2/3.3）、crossing 独立场景与 crossing-map（§5.1）、
> `follow-old-city-path`/`follow-handrail`/`leave-crossing` 等内容以
> `AGENTS.md` 的 "2026-08-05 merged old-city level" 决策为准。
