...
- Movement is restricted to brightly lit (warm) ground only: active color pulses (cane-tap radius 42 / 2s, spawn pulse, G flash) and forced-warm tiles are walkable; pale color memory and base gray are not. Exception (2026-08-05): the crossing walk state. Blocked moves announce a throttled hint instead of failing.
...
- `old-city` and `old-city-crossing` are merged into ONE level, `old-city`（白鸽巢 · 旧城街市）, rebuilt from the owner's hand-drawn map: a vertical road with a zebra band (rows 6–8) on the WEST side of the map; the player alights on the west sidewalk, requests crossing with E, crosses EAST over the zebra, then follows a U-shaped tactile route (south → east through the shop street → north) to the terminus in front of the bank, which transitions to the ruins. The crossing now happens at the START of the level, not the end.
- The handrail mechanic is CUT (GuideRailDefinition, rail interactions, `follow-handrail`, `old-city-rail` are all removed). The level's two interruption beats are the signal-controlled zebra crossing and the dirt dead-end alley (cols 22–24) between the tart shop and bakery, which hosts the `old-city-bell` memory (obstacle contact within 48px of `OLD_CITY_MEMORY_POINT`, or E nearby).
